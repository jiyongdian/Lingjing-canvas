...
} = require("./storage-optimization.cjs");
...
function beginProjectMigration(payload = {}) {
  ...
  try {
    const targetRoot = mediaLibraryRoot(directory);
    fs.mkdirSync(targetRoot, { recursive: true });
    fs.accessSync(targetRoot, fs.constants.R_OK | fs.constants.W_OK);
    const requiredBytes = Math.max(0, Number(payload.requiredBytes || 0));
    if (requiredBytes > 0) {
      const statfs = fs.statfsSync(targetRoot);
      const freeBytes = statfs.bavail * statfs.bsize;
      if (freeBytes < requiredBytes * 1.1) return { ok: false, error: "INSUFFICIENT_DISK_SPACE", freeBytes, requiredBytes };
    }
  } catch (error) {
    return { ok: false, error: "MIGRATION_DIRECTORY_UNAVAILABLE", detail: String(error?.message || error) };
  }
...
function syncProjectReferences(payload = {}) {
  ...
  restoreReferencedTrash({ directory, references });
...
function cleanupUnreferencedBlobs(payload = {}) {
  return moveUnreferencedToTrash(payload);
}

function isProjectMigrationLocked(payload = {}) {
  if (isStorageMaintenanceActive()) return true;
  const directory = payload.directory || defaultDownloadDirectory();
  const projectId = sanitizePathSegment(payload.projectId || "default", "default");
