...
const mediaClone = process.env.WANJUAN_FORMAL_MEDIA_CLONE;
...
if (!userData || !mediaClone || !migratedMedia || !resultPath) {
...
app.whenReady().then(async () => {
  ...
  const result = await win.webContents.executeJavaScript(`
    (async () => {
      ...
      const sourceRoot = ${JSON.stringify(sourceMediaRoot)};
      const cloneRoot = ${JSON.stringify(mediaClone)};
      const migratedRoot = ${JSON.stringify(migratedMedia)};
      const sourceUrlRoot = new URL("file://" + sourceRoot + "/").href.replace(/\\/$/, "");
      const cloneUrlRoot = new URL("file://" + cloneRoot + "/").href.replace(/\\/$/, "");
      const replacePaths = (value) => {
        if (typeof value === "string")
          return value.split(sourceRoot).join(cloneRoot).split(sourceUrlRoot).join(cloneUrlRoot);
        if (Array.isArray(value)) return value.map(replacePaths);
        if (!value || typeof value !== "object") return value;
        return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, replacePaths(item)]));
      };
      const projects = [];
      for (const project of chromeState.projects || []) {
        if (selected.size && !selected.has(project.id)) continue;
        const state = await globalThis.readProjectCanvasStorageState(project.id);
        if (!state) {
          projects.push({ projectId: project.id, name: project.name, error: "PROJECT_STATE_NOT_FOUND" });
          continue;
        }
        ...
        const rewritten = replacePaths(state);
        await globalThis.writeProjectCanvasStorageState(project.id, rewritten);
